...
void main(void){    
   ...
   // while(!OSCCONbits.IOFS);                // Wait for OSCON to become stable
    ...
    while(1){
        for (int i = -90; i < 90; i++){     // Increment from -90 to 90 degrees
            angle2PWM(i);
            __delay_ms(50);                 // Increment 1 degree every 0.05 seconds (~20 degrees per second)
        }
        for (int i = -90; i < 90; i++) {    // Reverse direction, as before
            angle2PWM(-1*i);
            __delay_ms(50);
        }
    }
